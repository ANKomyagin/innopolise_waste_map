...
class PostgresContainerRepo(ContainerRepository):
    ...
    async def update_location_coords(self, old_address: str, new_address: str, new_coords: str):
        ...
        async with SessionLocal() as db:
            ...
            # Убедимся, что координаты в правильном формате "lat, lon"
            coords_str = str(new_coords).strip()
            if ',' not in coords_str:
                raise ValueError(f"Координаты должны быть в формате 'lat, lon', получено: {new_coords}")
            
            for container in containers:
                container.address = new_address
                container.coords = coords_str
